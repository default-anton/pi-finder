...
This is the canonical publish procedure for `pi-finder-subagent`.

## Scope

- Release channel: **latest** only (stable tags `vX.Y.Z`).
- Publish target: npm package **`pi-finder-subagent`**.
...
6. **Post-release verification**
   - `npm view pi-finder-subagent version dist-tags --json`
   - `npm view` may lag due cache. Verify with registry data:
     ```bash
     curl -s https://registry.npmjs.org/pi-finder-subagent \
...
- If a bad version is published:
  1. Deprecate bad version:
     - `npm deprecate pi-finder-subagent@X.Y.Z "broken release; upgrade to >=X.Y.Z+1"`
